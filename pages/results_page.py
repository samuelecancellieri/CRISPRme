...
def resultPage(job_id: str) -> html.Div:
    ...
    value = job_id
    job_directory = os.path.join(current_working_directory, RESULTS_DIR, f"{job_id}")
    integrated_file_name = glob(
        os.path.join(
            current_working_directory, RESULTS_DIR, f"{job_id}", "*integrated*"
        )
    )[
        0
    ...
    guides_file = os.path.join(
        current_working_directory, RESULTS_DIR, f"{value}", ".guides.txt"
    )
    ...
    with open(
        current_working_directory + "Results/" + value + "/." + value + ".acfd.txt"
    ) as a:
    ...
    columns_profile_table = [
        ...
        {"name": ["", "CFD specificity score (0-100)"], "id": "CFD", "type": "text"},
        {
        ...
        final_list.append(
            dbc.Alert(
                [
                    ...
                    html.A(
                        "Click here",
                        href=URL + "/data/" + job_id + "/guides_error.txt",
...
def update_table_subset(
    page_current, page_size, sort_by, filter, hide_reference, search, hash_guide
):
    """
...
@app.callback(
    ...
    [Input("mm-dropdown", "value"), Input("general-profile-table", "selected_cells")],
...
def updateImagesTabs(mm, sel_cel, selection_criteria, search, all_guides):
    ...
    radar_img_encode_gencode = (
        ...
        + "."
        + str(bulge)
        + "_TOTAL.ENCODE+GENCODE.png"
    )
    os.system(
        f"python {app_main_directory}/PostProcess/generate_img_radar_chart.py {guide} {job_directory}/.guide_dict_{guide}.json {job_directory}/.motif_dict_{guide}.json {mm} {bulge} TOTAL {job_directory}/imgs/"
    )
        ...
        image_personal_top = "data:image/png;base64,{}".format(
            base64.b64encode(
                open(
                    ...
                    + "Results/"
                    + job_id
                    + f"/imgs/CRISPRme_top_1000_log_for_main_text_{guide}.{sample}.personal.png",
                    "rb",
                ).read()
            ).decode()
        )
        image_private_top = "data:image/png;base64,{}".format(
            base64.b64encode(
                open(
                    ...
                    + "Results/"
                    + job_id
                    + f"/imgs/CRISPRme_top_1000_log_for_main_text_{guide}.{sample}.private.png",
                    "rb",
                ).read()
            ).decode()
        )
...
@app.callback(
    ...
    [Input("tabs-reports", "value"), Input("general-profile-table", "selected_cells")],
    [
        State("general-profile-table", "data"),
        State("url", "search"),
...
def updateContentTab(value, sel_cel, selection_criteria, all_guides, search, genome_type):
    ...
    if (
        ...
        fl.append(html.Br())
        df = pd.read_csv(
            job_directory + job_id + ".summary_by_guide." + guide + ".txt",
            sep="\t",
            na_filter=False,
        )
        more_info_col = []
        ...
        try:
            if (
                nuclease == "SpCas9"
            ):  # IMPLEMENTARE DROPDOWN PER VISUALIZZARE CFD,CRISTA,MMBUL
                            ...
                            src="data:image/png;base64,{}".format(
                                base64.b64encode(
                                    open(
                                        ...
                                        + "Results/"
                                        + job_id
                                        + f"/imgs/CRISPRme_top_1000_log_for_main_text_{guide}.png",
                                        "rb",
                                    ).read()
                                ).decode()
                            ),
                            ...
                            src="data:image/png;base64,{}".format(
                                base64.b64encode(
                                    open(
                                        ...
                                        + "Results/"
                                        + job_id
                                        + f"/imgs/CRISPRme_top_1000_log_for_main_text_{guide}_MMvBUL.png",
                                        "rb",
                                    ).read()
                                ).decode()
                            ),
