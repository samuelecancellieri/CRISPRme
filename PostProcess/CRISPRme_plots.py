...
df["plot_AF"] =  df["AF"] + 0.001 # so haplotypes that got rounded down to AF = 0 (min AF = 0.01) still appear in the plot
...
df["color_str"] = np.where(df["gene_annotation"] == "CDS", "red", "gray") # coding
df["color_str"] = np.where(df["annotation"].str.contains("HSC-1", na=False), "blue", df["color_str"]) # DHS, accessible in HSCs
df["color"] = df["color_str"].apply(lambda x: transparent_red if x == "red" else (transparent_blue if x == "blue" else transparent_gray))

"""
Linear, annotated, top 100 (for supplement)
"""
# Plot data
ax = df.plot.scatter(x="index", y="highest_CFD_score(ref)", s="ref_AF", c="color", marker='*', zorder=1)
df.plot.scatter(x="index", y="highest_CFD_score(alt)", s="plot_AF", c="color", zorder=2, ax = ax)
...
for x,y,z in zip(df["index"], df["highest_CFD_score(ref)"], df["highest_CFD_score(alt)"]-df["highest_CFD_score(ref)"]):
    plt.arrow(x, y+0.01, 0, z-0.02, color='green', head_width=0.75, head_length=0.015, length_includes_head=True, alpha=0.5, zorder=0)
    # +/- to avoid overlap of arrow w/ points

# Size legend
s1 = mlines.Line2D([], [], marker='o', label='1', linestyle='None', markersize=math.sqrt(math.sqrt((1+0.001)*1000)), color='black')
s01 = mlines.Line2D([], [], marker='o', label='0.1', linestyle='None', markersize=math.sqrt(math.sqrt((0.1+0.001)*1000)), color='black')
s001 = mlines.Line2D([], [], marker='o', label='0.01', linestyle='None', markersize=math.sqrt(math.sqrt((0.01+0.001)*1000)), color='black')
plt.gca().add_artist(plt.legend(handles=[s1,s01,s001], title="Allele frequency", loc=9))

# Shape & color legend
star = mlines.Line2D([], [], marker='*', label='Reference', linestyle='None', markersize=10, color='black')
circle = mlines.Line2D([], [], marker='o', label='Alternative', linestyle='None', markersize=10, color='black')
...
# Plot data
ax = df.plot.scatter(x="index", y="highest_CFD_score(ref)", s="ref_AF", c=transparent_red, zorder=1)
# ax = df.plot.scatter(x="index", y="highest_CFD_score(ref)", s="ref_AF", c=transparent_red, zorder=1, ax=ax)
df.plot.scatter(x="index", y="highest_CFD_score(alt)", s="plot_AF", c=transparent_blue, zorder=2, ax=ax)
ax.set_xscale("log")
# plt.title("Top CRISPRme-identified sites for sgRNA 1617")
plt.xlabel("Candidate off-target site")
plt.ylabel("CFD score")
...
# Arrows
for x,y,z in zip(df["index"], df["highest_CFD_score(ref)"], df["highest_CFD_score(alt)"]-df["highest_CFD_score(ref)"]):
    plt.arrow(x, y+0.02, 0, z-0.04, color='gray', head_width=(x*(10**0.005-10**(-0.005))), head_length=0.02, length_includes_head=True, zorder=0, alpha=0.5)
    # +/- to avoid overlap of arrow w/ points, head_width calculated to remain constant despite log scale of x-axis

# Size legend
plt.gca().add_artist(plt.legend(handles=[s1,s01,s001], title="Allele frequency", ncol=3, loc=9))
