...
def shold(target,n_clicks,page_current,page_size,radio_order,orderdrop,sholddrop,maxdrop,asc1,url_job,guide, current_working_directory):
  ...
  if orderdrop!="None_(highest_CFD)": #ordinamento doppio
    if maxdrop==None: #min
      df=pd.read_sql_query("SELECT * FROM final_table WHERE \"{}\"=? AND {}>={} ORDER BY \"{}\" {},\"{}\" {} LIMIT ? OFFSET ?".format(guide_column,radio_order,sholddrop,radio_order,asc1,orderdrop,asc1),conn,params=param)
    else:#min e max
      df=pd.read_sql_query("SELECT * FROM final_table WHERE \"{}\"=? AND \"{}\" BETWEEN {} AND {} ORDER BY \"{}\" {},\"{}\" {}  LIMIT ? OFFSET ?".format(guide_column,radio_order,sholddrop,maxdrop,radio_order,asc1,orderdrop,asc1),conn,params=param)

  else:#ordinamento singolo
    if maxdrop==None: #min
        df=pd.read_sql_query("SELECT * FROM final_table WHERE \"{}\"=? AND \"{}\">={} ORDER BY \"{}\" {} LIMIT ? OFFSET ?".format(guide_column,radio_order,sholddrop,radio_order,asc1),conn,params=param)  #ok 
    else:#min e max
