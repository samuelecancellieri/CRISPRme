...
# REF/ALT_origin_(highest_CFD) DA USARE PER VERIFICARE SE REF o ALT ORIGIN
# create col in df with total count with equal value so ref target will have identical total
df['Mismatches+bulges_(highest_CFD)_REF'] = df['Mismatches+bulges_(highest_CFD)']
# print(df)

# if col is alt calculate total for ref, if ref skip
for index in df.index:
    if df.loc[index, 'REF/ALT_origin_(highest_CFD)'] == 'alt':
        refTarget = str(
            df.loc[index, 'Aligned_protospacer+PAM_REF_(highest_CFD)'])
        countMM = 0
        for nt in refTarget:
            if nt.islower():
                countMM += 1
        df.loc[index, 'Mismatches+bulges_(highest_CFD)_REF'] = countMM + \
            int(df.loc[index, 'Bulges_(highest_CFD)'])
...
# COLONNA TOTAL Mismatches+bulges_(highest_CFD)
# COLONNE TOTAL
# df['Mismatches+bulges_(highest_CFD)_REF'] = df['Mismatches+bulges_(highest_CFD)']
# Plot data
# ax = df.plot.scatter(x="index", y="CFD_score_REF_(highest_CFD)", s="ref_AF", c=transparent_red, zorder=1)
ax = df.plot.scatter(x="index", y="Mismatches+bulges_(highest_CFD)_REF",
...
df.plot.scatter(x="index", y="Mismatches+bulges_(highest_CFD)",
...
# Arrows
for x, y, z in zip(df["index"], df["Mismatches+bulges_(highest_CFD)"], df["Mismatches+bulges_(highest_CFD)_REF"]-df["Mismatches+bulges_(highest_CFD)"]):
...
plt.tight_layout()
plt.savefig(
    out_folder+f"CRISPRme_top_1000_log_for_main_text_{guide}_MMvBUL.png")
plt.clf()
