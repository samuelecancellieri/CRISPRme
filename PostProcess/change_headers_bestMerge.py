...
for chunk in chunks:
    ...
    chunk['Highest_CFD_aligned_protospacer+PAM_REF_corrected'] = np.where(mask, chunk['Highest_CFD_aligned_protospacer+PAM_ALT'], chunk['Highest_CFD_aligned_protospacer+PAM_REF'])
    chunk['Highest_CFD_aligned_protospacer+PAM_ALT'] = np.where(mask, chunk['Highest_CFD_aligned_protospacer+PAM_REF'], chunk['Highest_CFD_aligned_protospacer+PAM_ALT'])
    
    chunk['Highest_CFD_aligned_protospacer+PAM_REF'] = chunk['Highest_CFD_aligned_protospacer+PAM_REF_corrected']
    chunk.drop('Highest_CFD_aligned_protospacer+PAM_REF_corrected', axis = 1, inplace = True)
    
    mask = chunk['Fewest_mm+b_aligned_protospacer+PAM_REF'] == 'NA'
    chunk['Fewest_mm+b_aligned_protospacer+PAM_REF_corrected'] = np.where(mask, chunk['Fewest_mm+b_aligned_protospacer+PAM_ALT'], chunk['Fewest_mm+b_aligned_protospacer+PAM_REF'])
    chunk['Fewest_mm+b_aligned_protospacer+PAM_ALT'] = np.where(mask, chunk['Fewest_mm+b_aligned_protospacer+PAM_REF'], chunk['Fewest_mm+b_aligned_protospacer+PAM_ALT'])
    
    chunk['Fewest_mm+b_aligned_protospacer+PAM_REF'] = chunk['Fewest_mm+b_aligned_protospacer+PAM_REF_corrected']
    chunk.drop('Fewest_mm+b_aligned_protospacer+PAM_REF_corrected', axis = 1, inplace = True)
    
    chunk.to_csv(out_path, header = header, mode = 'a', sep = '\t', index=False) 
