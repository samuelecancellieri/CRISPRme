...
for guide in guides:
    ...
    with open(f"{path_output}/{name_job}.summary_by_guide.{guide}_{selection_criteria}.txt", "w") as summary_by_guide:
    ...
    if genome_type == "var":
        with open(f"{path_output}/{name_job}.summary_by_samples.{guide}_{selection_criteria}.txt", "w") as summary_by_samples:
    ...
    with open(f"{path_output}/.{name_job}.acfd_{selection_criteria}.txt", "a") as acfd:
        if sum_cfds == 0:
    ...
    df_general_count.to_csv(
        f"{path_output}/.{name_job}.general_target_count."+guide+"_"+selection_criteria+".txt", sep='\t', index=False)
    ...
    os.system(
        f"sort -k2,2n -k3,3n {path_output}/{name_job}.summary_by_guide.{guide}_{selection_criteria}.txt -o {path_best}.summary_by_guide.{guide}_{selection_criteria}.sorted")
    os.system(
        f"mv {path_best}.summary_by_guide.{guide}_{selection_criteria}.sorted {path_output}/{name_job}.summary_by_guide.{guide}_{selection_criteria}.txt")
    os.system(f"rm {path_best}.{guide}")


with open(f'{path_output}/.{name_job}.PopulationDistribution_{selection_criteria}.txt', 'w+') as pop_distribution:
    for g in guides:
