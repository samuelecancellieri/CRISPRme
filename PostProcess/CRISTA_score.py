...
def CRISTA_predict(sgseq_20nt, genomic_seq_29nt):
    ...
    for i in range(len(sgseq_20nt)):
        ...
        try:
            ...
            assert sgRNA_seq_re is not None and len(sgRNA_seq_re.group(
            )) == 20, "sgRNA sequence must be 20-nt long sequence of ACGTU nucleotides"
            full_dna_seq_re = re.search(
                "[acgtu]+", full_dna_seq, re.IGNORECASE)
            assert full_dna_seq_re is not None and len(full_dna_seq_re.group(
            )) == 29, "genomic sequence must be 29-nt long sequence of ACGTU nucleotides"
        except:
        ...
        features = get_features(full_dna_seq=full_dna_seq, aligned_sgRNA=aligned_sgRNA,
                                aligned_offtarget=aligned_offtarget, pa_score=max_score)
