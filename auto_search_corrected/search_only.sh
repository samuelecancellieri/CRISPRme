...
echo "Search Start:" $(date +%F-%T) >> $output_folder/log.txt
...
if [ "$vcf_name" != "_" ]; then
	if ! [ -d "dictionaries_$vcf_name/" ]; then
		echo "Dictionaries Start: "$(date +%F-%T) >> $output_folder/log.txt
...
if [ "$vcf_name" != "_" ]; then
	if ! [ -d "fake_chrom_$vcf_name" ]; then
		echo "INDELs Start: "$(date +%F-%T) >> $output_folder/log.txt
	...
	if ! [ -d "${ref_name}+${vcf_name}" ]; then
		echo "Add-variants Start: "$(date +%F-%T) >> $output_folder/log.txt
		...
		echo "Add-variants End: "$(date +%F-%T) >> $output_folder/log.txt
	else
...
if ! [ -d "genome_library/"$true_pam"_${bMax}_"$ref_name ]; then
	echo "Index-genome Reference Start: "$(date +%F-%T) >> $output_folder/log.txt	
	echo "Indexing reference genome"
	crispritz.py index-genome "$ref_name" "$ref_folder/" "$pam_file" -bMax $bMax -th $ncpus
	pid_index_ref=$!
	echo "Index-genome Reference End: "$(date +%F-%T) >> $output_folder/log.txt	
else
...
if [ "$vcf_name" != "_" ]; then
	if ! [ -d "genome_library/"$true_pam"_${bMax}_${ref_name}+${vcf_name}" ]; then
		echo "Index-genome Variant Start: "$(date +%F-%T) >> $output_folder/log.txt	
...
if ! [ -f "${ref_name}_${guide_name}_${mm}_${bDNA}_${bRNA}.targets.txt" ]; then
	echo "Search Reference Start: "$(date +%F-%T) >> $output_folder/log.txt	
...
if [ "$vcf_name" != "_" ]; then
	if ! [ -f "${ref_name}+${vcf_name}_${guide_name}_${mm}_${bDNA}_${bRNA}.targets.txt" ]; then
		echo "Search Variant Start: "$(date +%F-%T) >> $output_folder/log.txt	
		...
		echo "Search Variant End: "$(date +%F-%T) >> $output_folder/log.txt	
	else
...
echo "Search Reference End: "$(date +%F-%T) >> $output_folder/log.txt	
...
echo "Search End: "$(date +%F-%T) >> $output_folder/log.txt
echo "SEARCH END"
