...
inSamplesIguideDataFrameile = open(sys.argv[3], 'r').readlines()
inSamplesIguideDataFrameile.pop(0)
...
def generatePlot(guide, guideDict, motifDict, totalcount, source):
    ...
    plt.savefig("summary_single_guide_" + str(guide) + "_"+str(totalcount) + "_total"+'_'+source + "." + file_extension, format=file_extension)
...
for line in inSamplesIguideDataFrameile:
    split = line.strip().split('\t')
