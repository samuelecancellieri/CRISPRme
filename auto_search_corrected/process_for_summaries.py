...
os.system(f"cat {path_final_results_best} > {path_final_results_best}.cumulative")
os.system(f"tail -n +2 {path_final_results_alt} >> {path_final_results_best}.cumulative")
...
for guide in guides:
    os.system(f"grep {guide} {path_final_results_best}.cumulative > {path_final_results_best}.cumulative.{guide}")
    os.system(f"sort -k5,5 -k6,6n {path_final_results_best}.cumulative.{guide} -o {path_final_results_best}.cumulative.{guide}.sorted")
    dict_guide_summary = {}
    dict_samples_summary, dict_population, dict_superpopulation = init_dict_samples(path_samples)
    with open(f"{path_final_results_best}.cumulative.sorted.{guide}", 'r') as f:
        ...
        for line in f:
            ...
            if splitted[11] != 'n': #check pam_creation
                pam_creation = True
            class_target = splitted[0] + "\t" + splitted[8] + '\t' + splitted[9]    #  BulgeType\tMismatches\tBulges
