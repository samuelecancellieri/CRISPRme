...
def generatePlot(guide, guideDict, motifDict, totalcount, source):
    ...
    if guideDict['General'] == 0: #check if no targets are found for that combination source/totalcount and skip the execution
        return
    percentage_list = []
    for elem in guideDict:
        if float(guideDict['General']) != 0:
            percentage_list.append(float(str(float(guideDict[elem])/float(guideDict['General']))[0:5]))
        else:
    ...
    for count,elem in enumerate(categories):
        transpose_list.append([guideDataFrame.T.iloc[count]['Total'],guideDataFrame.T.iloc[count]['Percentage']])
    ...
    for count in range(len(guide)):
        for nuc in motifDict:
            if maxmax !=0:
                motifDict[nuc][count] = float(str(float(motifDict[nuc][count])/float(maxmax))[0:5])
    ...
    p5 = plt.bar(ind, RNA, width, color='magenta', bottom=C+G+A+T, align='edge')
    ...
    plt.legend((p1[0], p2[0], p3[0], p4[0], p5[0]),('A', 'C', 'G', 'T', 'RNA'), fontsize=18)
    ...
    plt.suptitle(str(totalcount)+" Mismatches + Bulge",horizontalalignment='center', color='black', size=25)

    plt.tight_layout()
    plt.subplots_adjust(top=0.90, bottom=0.07, left=0.09,right=0.99, wspace=0.25)
    plt.savefig(outDir+"/summary_single_guide_" + str(guide) + "_"+str(totalcount) + "_total"+'_'+source + "." + file_extension, format=file_extension)
        ...
        if 'DNA' not in split[0]:
            for count, nucleotide in enumerate(alignedSequence):
                if nucleotide.islower():
                    motifDict[totalcount]['REF'][nucleotide.upper()][count] +=1
                elif nucleotide == '-':
            ...
            for sample in samplePresent:
                ...
                if superpop not in alreadySampled: #check if superpop is already been updated for the current target to avoid duplicate count
                    ...
                    if 'DNA' not in split[0]:
                        for count, nucleotide in enumerate(alignedSequence):
                            if nucleotide.islower():
                                motifDict[totalcount][superpop][nucleotide.upper()][count] +=1
                            elif nucleotide == '-':
                                motifDict[totalcount][superpop][split[0]][count] +=1
                    if split[14] != 'n':
                        annotationsList = split[14].strip().split(',')
                        for annotation in annotationsList:
                ...
                if pop not in alreadySampled: #check if pop is already been updated for the current target to avoid duplicate count
                    ...
                    if 'DNA' not in split[0]:
                        for count, nucleotide in enumerate(alignedSequence):
                            if nucleotide.islower():
                                motifDict[totalcount][pop][nucleotide.upper()][count] +=1
                            elif nucleotide == '-':
                                motifDict[totalcount][pop][split[0]][count] +=1
                    if split[14] != 'n':
                        annotationsList = split[14].strip().split(',')
                        for annotation in annotationsList:
                            guideDict[totalcount][pop][annotation] += 1
                guideDict[totalcount][sample]['General'] += 1 #add target to single sample, in this case duplicate are allowed
                if 'DNA' not in split[0]:
                    for count, nucleotide in enumerate(alignedSequence):
                        if nucleotide.islower():
                            motifDict[totalcount][sample][nucleotide.upper()][count] +=1
                        elif nucleotide == '-':
...
for guide in inGuideFile:
    ...
    if __name__ == '__main__':
        ...
        for totalcount in range(0,9):
            for elem in guideDict[totalcount]:
                pool.apply_async(generatePlot, args=(guide,guideDict[totalcount][elem],motifDict[totalcount][elem],totalcount,elem))
        pool.close()
        pool.join()
    # for totalcount in range(0,9):
    #     p = Pool(9)
